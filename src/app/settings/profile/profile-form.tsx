  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-accent-gold/40 bg-gradient-to-b from-accent-gold/8 to-surface p-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="font-mono text-[12px] text-text-muted">{email}</p>
            <div className="mt-1 flex items-center gap-2">
              {isAdmin ? (
                <span className="rounded border border-accent-gold/50 bg-accent-gold/15 px-2 py-0.5 font-mono text-[12px] font-bold uppercase tracking-widest text-accent-gold-text">
                  Admin
                </span>
              ) : (
                <span className="rounded border border-accent-cyan/50 bg-accent-cyan/10 px-2 py-0.5 font-mono text-[12px] font-bold uppercase tracking-widest text-accent-cyan">
                  Premium
                </span>
              )}
              {effectiveTitle.trim() && (
                <span className="rounded border border-accent-gold/30 bg-surface px-2 py-0.5 font-mono text-[12px] text-accent-gold-text">
                  {effectiveTitle}
                </span>
              )}
            </div>
        ...
        <p className="mt-3 border-t border-accent-gold/10 pt-3 font-mono text-[12px] text-text-muted">
          Member since{" "}
      ...
      <div className="rounded-xl border border-border bg-surface p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
            {PRESET_TITLES.map((t) => (
              <button
                key={t}
                onClick={() => setTitle(t)}
                className={`rounded-lg border px-3 py-2 text-left font-mono text-[12px] transition-all ${
                  title === t
        ...
        {useCustom && (
          <input
            ...
            className="w-full rounded-lg border border-border bg-elevated px-4 py-2.5 font-mono text-sm text-text-primary placeholder:text-text-muted focus:border-accent-gold/50 focus:outline-none focus:ring-1 focus:ring-accent-gold/30"
          />
        )}
      ...
      <div className="rounded-xl border border-border bg-surface p-6">
        ...
        {isPublic && (
          <p className="mt-3 font-mono text-[12px] text-accent-cyan/80">
      ...
      {isSystemTier && (
        <div className="mt-2 space-y-5 rounded-xl border border-accent-violet/30 bg-gradient-to-b from-accent-violet/5 to-surface p-6">
          <div>
            <p className="font-mono text-[12px] uppercase tracking-[0.12em] text-accent-violet-text/70 mb-1">
          ...
          <div className="space-y-1.5">
            <label className="font-mono text-[12px] uppercase tracking-wider text-text-muted">Your Page URL</label>
            <div className="flex items-center">
              <span className="rounded-l-lg border border-r-0 border-border bg-elevated px-3 py-2.5 font-mono text-[12px] text-text-muted whitespace-nowrap">
              ...
              <input
                ...
                className="flex-1 rounded-r-lg border border-border bg-elevated px-3 py-2.5 font-mono text-sm text-text-primary placeholder:text-text-muted focus:border-accent-violet/50 focus:outline-none focus:ring-1 focus:ring-accent-violet/30 min-w-0"
              />
            </div>
            ...
            {codexSlug.length >= 3 && (
              <p className="font-mono text-[12px] text-accent-violet-text/70">
                Preview:{" "}
          ...
          <div className="space-y-2">
            <label className="font-mono text-[12px] uppercase tracking-wider text-text-muted">Page Banner Theme</label>
            <div className="grid grid-cols-4 gap-2">
              {Object.entries(BANNER_THEMES).map(([key, theme]) => (
                <button
                  ...
                  style={theme.style}
                >
                  <span className="absolute inset-x-0 bottom-0 pb-1 text-center font-mono text-[12px] uppercase tracking-wider" style={{ color: theme.accent }}>
                    {theme.label}
                  </span>
                </button>
              ))}
          ...
          {/* Bio */}
          <div className="space-y-1.5">
            <label className="font-mono text-[12px] uppercase tracking-wider text-text-muted">Bio</label>
            <textarea
              value={bio}
              onChange={(e) => setBio(e.target.value.slice(0, 500))}
              rows={4}
              placeholder="Tell the archive who you are..."
              className="w-full rounded-lg border border-border bg-elevated px-4 py-2.5 font-mono text-sm text-text-primary placeholder:text-text-muted focus:border-accent-violet/50 focus:outline-none focus:ring-1 focus:ring-accent-violet/30 resize-none"
            />
            <p className="font-mono text-[12px] text-text-muted text-right">{bio.length}/500</p>
          </div>

          {/* Social links */}
          <div className="space-y-2">
            <label className="font-mono text-[12px] uppercase tracking-wider text-text-muted">Social Links</label>
            <p className="font-mono text-[12px] text-text-muted">Up to 5 links. These appear on your public page.</p>
            <div className="space-y-2">
              {links.map((link, i) => (
                <div key={i} className="flex gap-2">
                  <div className="relative">
                    <input
                      ...
                      className="w-28 rounded-lg border border-border bg-elevated px-2.5 py-2 font-mono text-[12px] text-text-primary placeholder:text-text-muted focus:border-accent-violet/50 focus:outline-none"
                    />
                  </div>
                  <input
                    type="url"
                    ...
                    className="flex-1 rounded-lg border border-border bg-elevated px-2.5 py-2 font-mono text-[12px] text-text-primary placeholder:text-text-muted focus:border-accent-violet/50 focus:outline-none min-w-0"
                  />
                  <button
                    onClick={() => removeLink(i)}
                    className="flex-shrink-0 rounded-lg border border-border bg-elevated px-2.5 py-2 font-mono text-[12px] text-text-muted hover:border-red-500/50 hover:text-red-400 transition-colors"
            ...
            {links.length < 5 && (
              <button
                onClick={addLink}
                className="flex items-center gap-2 rounded-lg border border-dashed border-accent-violet/30 px-4 py-2 font-mono text-[12px] text-accent-violet-text/70 hover:border-accent-violet/60 hover:text-accent-violet-text transition-colors"
              >
                + Add link
              </button>
            )}
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              <h4 className="font-mono text-[12px] font-bold text-text-primary">Show Card Collection</h4>
          ...
          <div className="flex items-start justify-between gap-4 pt-1 border-t border-accent-violet/10">
            <div>
              <h4 className="font-mono text-[12px] font-bold text-text-primary">Public page</h4>
              <p className="font-mono text-[12px] text-text-muted">
          ...
          <div className="flex items-center justify-between pt-1">
            <div>
              {codexError && <p className="font-mono text-[12px] text-red-400">{codexError}</p>}
              {codexSaved && <p className="font-mono text-[12px] text-green-400">✓ Codex page saved</p>}
            </div>
            <button
