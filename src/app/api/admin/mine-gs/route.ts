...
export async function GET(req: NextRequest) {
  ...
  const [humorRaw, prophecies, quotesRaw, castRaw, locations, artifacts, trollRaw, peopleRaw] = await Promise.all([
    ...
    prisma.quote.findMany({ where: { speakerPersonId: { not: null }, text: { not: "" } }, select: { text: true, speaker: { select: { displayName: true } }, episode: { select: { slug: true, title: true } } }, take: 1500, orderBy: { id: "asc" } }),
    ...
    prisma.person.findMany({ where: { personType: { in: ["recurring", "host"] }, OR: [{ shortBio: { not: null } }, { loreSummary: { not: null } }] }, select: { displayName: true, slug: true, shortBio: true, loreSummary: true, personType: true }, take: 120 }),
  ]);
  ...
  return NextResponse.json({ humor, prophecies, quotes, cast, locations: locations.filter((l) => ok(l.title)), artifacts: artifacts.filter((a) => ok(a.title)), troll, people });
}
