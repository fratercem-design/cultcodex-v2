...
async function generateChapter(req: NextRequest) {
  ...
  if (existing) {
    return NextResponse.json({ error: "Chapter already exists for this episode", chapterId: existing.id }, { status: 409 });
  }
...
Generate Chapter ${nextChapterNumber} of the Psychenomicon. Output ONLY valid JSON with this exact shape:
{
  ...
  for (const e of generated.entities ?? []) {
    ...
    if (existing) {
      archetypeHistory = (existing.archetypeHistory as typeof archetypeHistory) ?? [];
    }
    ...
    const entity = await prisma.psychenomiconEntity.upsert({
      ...
      update: {
        ...
        archetypeHistory,
        radarData: e.traits ?? undefined,
        behaviorPatterns: e.behaviorPatterns?.length ? e.behaviorPatterns : undefined,
        updatedAt: new Date(),
      },
      select: { id: true },
    });
  ...
  const chapter = await prisma.psychenomiconChapter.create({
    data: {
      ...
      threadChapters: {
        create: threadIds.map((threadId) => ({ threadId })),
      },
    },
