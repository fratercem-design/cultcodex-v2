      ...
      <section className="border-b border-accent-violet/20 bg-gradient-to-b from-accent-violet/5 to-void py-10 px-4">
        <div className="mx-auto max-w-4xl flex flex-col sm:flex-row sm:items-end justify-between gap-4">
          <div className="space-y-2">
            <Link href="/psychenomicon" className="font-mono text-[12px] uppercase tracking-[0.12em] text-accent-violet-text/70 hover:text-accent-violet-text transition-colors">
              ← The Psychenomicon
            </Link>
      ...
      <div className="mx-auto max-w-4xl px-4 py-8 space-y-8">
        {!canRead && (
          <div className="rounded border border-accent-gold/30 bg-accent-gold/5 px-4 py-3 flex flex-wrap items-center justify-between gap-2">
            <p className="font-mono text-[12px] text-text-muted">
              <span className="uppercase tracking-[0.12em] text-accent-gold-text mr-2">{"/// free_preview"}</span>
              A few chapters are unsealed for all. The full chronicle is Initiate+.
            </p>
            <Link href="/premium#access" className="font-mono text-[12px] font-bold text-accent-gold-text hover:underline">
        ...
        {groups.map((g) => (
          <section key={g.label} className="space-y-2">
            <p className="font-mono text-[12px] uppercase tracking-[0.12em] text-accent-gold-text/80 sticky top-0 bg-void/90 backdrop-blur-sm py-1.5 z-10">
              {g.label}
            </p>
                ...
                return (
                  ...
                  >
                    ...
                    {c.episode?.episodeNumber != null && (
                      <span className="font-mono text-[12px] text-text-muted flex-shrink-0 hidden sm:inline">
                        EP.{String(c.episode.episodeNumber).padStart(3, "0")}
                      </span>
                    )}
                    {!canRead && (
                      free ? (
                        <span className="flex-shrink-0 rounded border border-accent-gold/40 bg-accent-gold/10 px-1.5 py-0.5 font-mono text-[12px] font-bold uppercase text-accent-gold-text">
                          Free
                        </span>
                      ) : (
                        <span className="flex-shrink-0 font-mono text-[12px] text-text-muted" aria-label="Initiate+ only">
                          🔒
                        </span>
                      )
                    )}
        ...
        <nav className="flex items-center justify-between gap-3 border-t border-border pt-6" aria-label="Pagination">
          {page > 1 ? (
            <Link href={pageHref(page - 1)} className="font-mono text-[12px] uppercase tracking-widest text-accent-violet-text hover:underline">
              ← Earlier
            </Link>
          ) : <span />}
          <span className="font-mono text-[12px] text-text-muted">{page} / {totalPages}</span>
          {page < totalPages ? (
            <Link href={pageHref(page + 1)} className="font-mono text-[12px] uppercase tracking-widest text-accent-violet-text hover:underline">
              Later →
            </Link>
