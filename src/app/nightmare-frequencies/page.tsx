...
export const metadata: Metadata = {
  alternates: { canonical: "/nightmare-frequencies" },
  title: "Nightmare Frequencies — CultCodex",
  description: "Latest videos and live streams from Nightmare Frequencies (@NightmareFrequenciesTV).",
};
...
export default async function NightmareFrequenciesPage() {
  ...
  return (
    <main id="main-content" className="mx-auto max-w-7xl px-4 py-8 space-y-8">
      ...
      <div className="space-y-1">
        <p className="font-mono text-[10px] uppercase tracking-[0.4em] text-accent-cyan/60">{"/// nightmare_frequencies"}</p>
        <div className="flex items-center gap-3">
          <h1 className="font-display text-2xl font-bold text-white">Nightmare Frequencies</h1>
          {liveCount > 0 && (
        ...
        {allVideos.map((v) => {
          ...
          return (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="group flex flex-col rounded-xl border border-border bg-surface overflow-hidden hover:border-accent-cyan/40 transition-colors"
            >
              {/* Thumbnail */}
              <div className="relative w-full overflow-hidden" style={{ paddingBottom: "56.25%" }}>
                {v.thumbnailUrl ? (
                  ...
                  <div className="absolute inset-0 bg-gradient-to-br from-accent-cyan/10 to-void" />
                )}
                {v.isLive && (
              ...
              <div className="p-3 space-y-1 flex-1">
                ...
                <p className="text-xs font-medium text-text-primary leading-snug line-clamp-2 group-hover:text-accent-cyan transition-colors">
                  {v.title}
                </p>
