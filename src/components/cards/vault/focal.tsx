...
const M: Record<string, FocalFn> = {
  ...
  waveform: (c, u) => (<g filter={`url(#${u}-fd)`}>{[...Array(15)].map((_, i) => { const h = Math.abs(sin(i * 0.8) * cos(i * 0.4)) * 30 + 4; return <rect key={i} x={22 + i * 4} y={50 - h / 2} width="2.6" height={h} rx="1.3" fill={c} opacity="0.85" />; })}<line x1="20" y1="50" x2="82" y2="50" stroke={c} strokeWidth="0.5" opacity="0.4" /></g>),
  ouroboros: (c, u) => (<g filter={`url(#${u}-fg)`}>{[...Array(40)].map((_, i) => { const a = (i / 40) * 6.283; const r = 26 + sin(i * 0.9) * 1.5; return <circle key={i} cx={50 + cos(a) * r} cy={50 + sin(a) * r} r={1 + (i / 40) * 1.6} fill={i > 35 ? "#0a0402" : "#ff5a1e"} opacity={i > 32 ? 0.4 : 0.9} />; })}<circle cx="50" cy="50" r="20" fill="#000" /></g>),
