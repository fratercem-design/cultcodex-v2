...
function mShort(d: string | number | Date | null | undefined) { if (!d) return "Undated"; const dt = new Date(d); return MONTHS[dt.getMonth()] + " " + dt.getFullYear(); }
...
type ChronicleChapter = {
  ...
  episode?: { airDate?: string | null } | null;
};
type ChronicleData = {
  canRead: boolean;
  isAdmin: boolean;
...
export default function PsychenomiconChronicle() {
  ...
  const arcGroups: { key: string; label: string; short: string; rows: ChronicleChapter[] }[] = [];
