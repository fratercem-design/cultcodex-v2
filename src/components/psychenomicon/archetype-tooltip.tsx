      ...
      {open && (
        <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 z-50 pointer-events-none">
          <span className="block w-56 rounded-lg border border-accent-violet/30 bg-void/98 backdrop-blur-sm p-3 shadow-2xl shadow-accent-violet/10 space-y-2">
            <span className="block font-mono text-[12px] uppercase tracking-[0.12em] text-accent-violet-text/70">{"/// entity"}</span>
            <span className="block font-mono text-xs font-bold text-text-primary">{entity.name}</span>
            {entity.primaryArchetype && (
              <span className="block font-mono text-[12px] text-accent-violet-text">{entity.primaryArchetype}</span>
            )}
            {entity.behaviorPatterns && entity.behaviorPatterns.length > 0 && (
              <span className="block flex flex-wrap gap-1 mt-1">
                {entity.behaviorPatterns.slice(0, 3).map((p) => (
                  <span key={p} className="inline-block rounded border border-border px-1.5 py-0.5 font-mono text-[12px] text-text-muted">
                    {p}
                  </span>
                ))}
              </span>
            ...
            <Link
              ...
              className="pointer-events-auto block font-mono text-[12px] text-accent-violet-text/70 hover:text-accent-violet-text transition-colors mt-1"
            >
              View full profile →
            </Link>
          </span>
