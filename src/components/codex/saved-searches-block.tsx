    ...
    <section className="space-y-5">
      <div className="flex flex-wrap items-baseline justify-between gap-3">
        <div className="space-y-1">
          ...
          <h2 className="font-display text-xl font-bold text-accent-violet-text">
            Searches{" "}
      ...
      {all.length === 0 ? (
        <div className="rounded-lg border border-dashed border-border bg-surface/50 p-8 text-center space-y-3">
          <p className="text-sm text-text-muted">
            No saved searches yet. Save complex queries from <Link href="/search/deep" className="text-accent-violet-text hover:underline">Deep Search</Link> or the Oracle.
          </p>
          <Link
            href="/search/deep"
              ...
              >
                <div className="min-w-0 flex-1 space-y-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    {s.pinned && (
                      <span className="text-[12px] text-accent-violet-text" aria-label="Pinned">★</span>
                    )}
