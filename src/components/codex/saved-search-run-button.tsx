...
export function SavedSearchRunButton({ id, kind, payload }: Props) {
  ...
  return (
    <button
      type="button"
      onClick={run}
      className="shrink-0 rounded border border-accent-violet/30 px-2.5 py-1 font-mono text-[12px] uppercase tracking-widest text-accent-violet-text hover:bg-accent-violet/10 transition-colors"
    >
      Run →
    </button>
  );
