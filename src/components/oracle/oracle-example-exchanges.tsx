...
export function OracleExampleExchanges() {
  ...
  return (
    <div className="space-y-3">
      <p className="font-mono text-[9px] uppercase tracking-[0.35em] text-accent-violet-text/70 text-center">
      ...
      <div className="space-y-2">
        {EXCHANGES.map((ex, i) => (
          ...
          >
            ...
            >
              <span className="mt-0.5 shrink-0 font-mono text-[9px] text-accent-violet-text/70 uppercase tracking-widest pt-0.5">Q</span>
              <span className="font-mono text-xs text-text-primary leading-relaxed flex-1">{ex.q}</span>
              <span className={`shrink-0 font-mono text-[10px] text-accent-violet-text/70 transition-transform ${open === i ? "rotate-180" : ""}`}>▾</span>
            </button>
