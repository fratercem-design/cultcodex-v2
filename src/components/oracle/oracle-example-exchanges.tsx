...
// Order matters: the sample open by default is the topic-level one. The two
// samples that characterise named individuals follow it and should be
// replaced with topic-level questions (real Oracle outputs) — publicly
// profiling private guests as marketing is flagged in the 2026-09 audit
// (OR-03).
const EXCHANGES = [
  {
...
export function OracleExampleExchanges() {
  ...
  return (
    <div className="space-y-3">
      ...
      <p className="text-center font-mono text-[12px] text-text-muted mb-4">
      ...
      <div className="space-y-2">
        {EXCHANGES.map((ex, i) => (
          ...
          >
            ...
            >
              <span className="mt-0.5 shrink-0 font-mono text-[12px] text-accent-violet-text/70 uppercase tracking-widest pt-0.5">Q</span>
              <span className="font-mono text-xs text-text-primary leading-relaxed flex-1">{ex.q}</span>
              <span className={`shrink-0 font-mono text-[12px] text-accent-violet-text/70 transition-transform ${open === i ? "rotate-180" : ""}`}>▾</span>
            </button>
            ...
            {open === i && (
              <div className="px-5 pb-5 space-y-4 border-t border-border/50">
                <div className="flex gap-3 pt-4">
                  <span className="mt-0.5 shrink-0 font-mono text-[12px] text-accent-cyan/60 uppercase tracking-widest">◈</span>
                ...
                <div className="flex flex-wrap gap-2 pl-5">
                  {ex.citations.map((c) => (
                    <span
                      key={c}
                      className="rounded border border-accent-violet/15 bg-void px-2 py-0.5 font-mono text-[12px] text-text-muted"
                    >
                      {c}
                    </span>
                  ))}
      ...
      <p className="text-center font-mono text-[12px] text-text-muted pt-2">
