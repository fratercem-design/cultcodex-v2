...
function buildMahavidyaPrompt(title: string, subtitle: string, flavourText: string, rarity: string): string {
  const iconography: Record<string, string> = {
    ...
    "Chinnamastā": "radiant red-golden complexion, standing in a victorious cosmic pose with arms raised, three luminous golden halos of sacred energy radiating from her crown outward and downward to bless two kneeling devotees below, wearing a garland of lotus emblems and gems, lotus throne beneath her feet, blazing aura of liberation energy, surrounded by sacred geometry and divine light, traditional Tantric devotional painting, extremely detailed, no gore, sacred museum art",
  ...
  const icon = iconography[title] ?? `${title}, ${subtitle}, fierce divine goddess, multiple arms, divine weapons, traditional sacred iconography`;
