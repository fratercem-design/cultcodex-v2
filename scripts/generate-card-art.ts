...
function buildMahavidyaPrompt(title: string, subtitle: string, flavourText: string, rarity: string): string {
  const iconography: Record<string, string> = {
    ...
    "Chinnamastā": "radiant red-golden complexion, cosmic goddess in a posture of supreme self-sacrifice, holding aloft her own crowned head — symbol of ego transcendence — three sacred streams of life-force flowing upward, two devotees receiving the nectar of liberation, standing on a lotus above a yin-yang pair symbolising union, blazing aura, traditional tantric devotional painting",
